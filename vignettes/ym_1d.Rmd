...
```{r}
compute_perplexity <- function(model, data) {
  bigrams <- data %>%
    add_start_token() %>%
    unnest_tokens(pair, text, token = "ngrams", n = 2) %>%
    separate(pair, into = c("w1", "w2"), sep = " ") %>%
  ...
  matches <- bigrams %>%
    ...
    summarize(sum_log_probs = sum(log_prob)) %>%
    mutate(prod_probs = 2^sum_log_probs,
           inv_prod_probs = 1 / prod_probs,
           nth_root = inv_prod_probs^(1/N))
  
  matches
}
data.frame(text = c("I want to go fishing")) %>%
  compute_perplexity(ym_1b_news, .)
