...
mtcars %>%
  modelr::resample_partition(p = c(train = 0.80, test = 0.20)) %>%
  enframe() %>%
  spread(name, value)
...
tribble(
  ...
  mutate(split = map(corpora, ~ enframe(modelr::resample_partition(.x, p = c(train = 0.80, test = 0.20)))  %>% spread(name, value))) %>%
  unnest(split)
```
...
```{r}
tidy_news <- texts[texts$source == "News", "corpora"][[1]][[1]] %>%
  mutate(source = "Blogs")
tidy_blogs <- texts[texts$source == "Blogs", "corpora"][[1]][[1]]
tidy_twitter <- texts[texts$source == "Twitter", "corpora"][[1]][[1]]
